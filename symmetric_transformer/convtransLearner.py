...
class convtransLearner(object):
    ...
    def train(self, num_epoch, train_loader, valid_loader, test_loader):
        ...
        for epoch in range(num_epoch):
            ...
            outputtest = ('Epoch={}, time {:.4f}, train loss {:.4f} '.format(epoch, seconds, total_loss)
                    +  'test auc {:.4f}'.format(auctest))
            print (outputtest)
            with open(self.filename, 'a') as outfile:
                outfile.write(outputtest+'\n')
            #self.scheduler.step(auc)
            
            
            torch.save(self.model, self.model_name+str(epoch))
    ...
    def evalpositive(self, data_loader, form='mean'):
        self.model.eval()
        all_score = []
        all_y = []
        for batch_x, batch_y in data_loader:
        ...
        all_score = np.concatenate(all_score)
        all_y= np.concatenate(all_y)
        return evaluatepositive(all_y, all_score, form)




    def evalreverse(self, data_loader, form='mean'):
        self.model.eval()
        all_score = []
        all_y = []
        for batch_x, batch_y in data_loader:
        ...
        all_score = np.concatenate(all_score)
        all_y= np.concatenate(all_y)
        return evaluatereverse(all_y, all_score, form)

    def evalroc(self, data_loader, form='mean'):
        self.model.eval()
        all_score = []
        all_y = []
        for batch_x, batch_y in data_loader:
        ...
        return evaluateroc(all_y, all_score, form)
    ...
    def predictnew(self, data_loader):
        self.model.eval()
        all_score = []
        #all_y = []
        for batch_x, batch_y in data_loader:
