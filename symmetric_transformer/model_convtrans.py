...
class EncoderModel(nn.Module):
    ...
    def __init__(self, conv, pool, encoder1, encoder2, position_emb, n_class, d_model):
        ...
        self.lay_nn = nn.Linear(2*d_model, d_model)
    ...
    def forward(self, src):
        ...
        tmp21[:, 0, 0] = 1
        tmp12 = torch.from_numpy(np.zeros((len(x2), 1023, 1), dtype=np.float32)).float().to(devices)
        tmp22 = torch.from_numpy(np.zeros((len(x2), 1, 76), dtype=np.float32)).float().to(devices)
        tmp22[:, 0, 0] = 1
        
        x1 = torch.cat((tmp11, x1), 2)
        x1 = torch.cat((tmp21, x1), 1)
        x1 = self.pos_emb(torch.transpose(x1, 1, 2))
        x1 = self.encode1(x1)
        x1 = x1[:, 0, :] #Get the special token representation
        
        x2 = torch.cat((tmp12, x2), 2)
        x2 = torch.cat((tmp22, x2), 1)
